...
double RunSimulationRel(int tskitstatus, bool isabsolute, bool ismodular, int elementsperlb, char * Nxtimestepsname, char * popsizename, char * delmutratename, char * chromsizename, char * chromnumname, char * mubname, char * Sbname, int typeofrun, int Nxtimesteps, int popsize, int chromosomesize, int numberofchromosomes, double deleteriousmutationrate, double beneficialmutationrate, double Sb, int beneficialdistribution, double Sd, int deleteriousdistribution, gsl_rng * randomnumbergeneratorforgamma, FILE *miscfilepointer, FILE *veryverbosefilepointer, int rawdatafilesize)
{
    ...
    fprintf(rawdatafilepointer, "Nxtimesteps,Sum.of.wis,Variance.in.log.fitness,Fittest.wi\n");
    ...
    // Variable which will be used to scale fitness with renormalization; initialized to 1
    ...
    long double currentfittestindividualswi = 1.0; // All individuals start with the same fitness, wi = 1.0, so this must be the current fittest individual's wi
    ...
    for (i = 0; i < Nxtimesteps; i++) {
        ...
        // If necessary, the fitnesses are re-normalized
        if (i % RENORM_FREQ == 0 && i != 0) {
            renormalizeFitness(wholepopulationgenomes, wholepopulationwisarray, sortedwisarray, psumofwis, wholepopulationwistree, totalpopulationgenomelength, popsize, miscfilepointer, totalindividualgenomelength, &renorm_factor);
        }

        // We also capture the current state of the tree sequence, if tskit is on and if we're in the appropriate generation
        if (tskitstatus > 0 && i % CAPTURE_FREQ == 0 && i != Nxtimesteps - 1 && i != 0 && i >= 100000) {
            captureCurrentTskitState(&treesequencetablecollection, wholepopulationnodesarray, i, popsize);
        }
        ...
        fprintf(rawdatafilepointer, "%d,%Lf,%.18f,%Lf\n", i+1, *psumofwis, variancesum, currentfittestindividualswi);
...
// Updated to account for renormalization- also now involves multiplying by a renormalization factor, to account for 
// renormalization in the code
double CalculateWi(double *parent1gamete, double *parent2gamete, int totalindividualgenomelength, double renorm_factor)
{
    double newwi = 0.0;
...
// Function used to renormalize fitness, as is necessary, to prevent it from tanking and crashing the 
// code prematurely. Note that to ensure that new individuals being added into the population possess
// this renormalized fitness as well, a global variable which begins at 1, but which is updated with
// each renormalization, is used to scale fitness.
void renormalizeFitness(double *wholepopulationgenomes, long double *wholepopulationwisarray, long double *sortedwisarray, long double *psumofwis, long double *wholepopulationwistree, int totalpopulationgenomelength, int popsize, FILE *miscfilepointer, int totalindividualgenomelength, double *renorm_factor) {
    // First, gets the current mean fitness
    ...
    *psumofwis = 0;
    ...
    // Updates the renormalization factor, dividing it by the mean fitness as well
    *renorm_factor = *renorm_factor / mean_fitness;

    // Updates the underlying fitness contributions, in order to ensure that these are also re-normalized
    for (int i = 0; i < totalpopulationgenomelength; i++) {
        wholepopulationgenomes[i] += log(*renorm_factor) / (totalindividualgenomelength);
    }
}

// Function used to capture the current state of the tree sequence with tskit, which can be used to
// output intermediate tree sequences in order to determine whether the SFS/related summary statistics
// are converging within the course of a given run. When called, writes new tskit output files for the 
// current state of the tree sequence, after first cleaning this tree sequence. Note that the combination
// of tree cleaning and heavy file I/O means that this method is quite costly to run time-wise, and the 
// output files make it space-intensive as well, so it should be run sparingly
void captureCurrentTskitState(tsk_table_collection_t *treesequencetablecollection, tsk_id_t *wholepopulationnodesarray, int generation_number, int popsize) {
    // First, because it's unlikely to be the case that the simplify interval matches our choice of when
    // to output, we sort and simplify the tree sequence to ensure that it's ready to be read to file
    int returnvaluefortskfunctions = tsk_table_collection_sort(treesequencetablecollection, NULL, 0);
    check_tsk_error(returnvaluefortskfunctions);
    ...
    // We then construct new files to store the node, edge, site, and mutation tables
    char *nodefilename = malloc(100);
    char *edgefilename = malloc(100);
    char *sitefilename = malloc(100);
    char *mutationfilename = malloc(100);

    snprintf(nodefilename, 100, "nodetable_gen%d.txt", generation_number);
    snprintf(edgefilename, 100, "edgetable_gen%d.txt", generation_number);
    snprintf(sitefilename, 100, "sitetable_gen%d.txt", generation_number);
    snprintf(mutationfilename, 100, "mutationtable_gen%d.txt", generation_number);

    FILE *nodefilepointer = fopen(nodefilename, "w");
    FILE *edgefilepointer = fopen(edgefilename, "w");
    FILE *sitefilepointer = fopen(sitefilename, "w");
    FILE *mutationfilepointer = fopen(mutationfilename, "w");

    //Printing out the node table in a way readable by python
    fprintf(nodefilepointer, "is_sample time\n");
    for (int k = 0; k <= treesequencetablecollection->nodes.num_rows; k++) {
        if (k < (2*popsize)) {
            fprintf(nodefilepointer, "1 %f\n", treesequencetablecollection->nodes.time[k]);
        } else {
            fprintf(nodefilepointer, "0 %f\n", treesequencetablecollection->nodes.time[k]);
        }
    }

    //Printing out the edge table in a way readable by python
    fprintf(edgefilepointer, "left right parent child\n");
    for (int k = 0; k < treesequencetablecollection->edges.num_rows; k++) {
        fprintf(edgefilepointer, "%f %f %d %d\n", treesequencetablecollection->edges.left[k], treesequencetablecollection->edges.right[k], treesequencetablecollection->edges.parent[k], treesequencetablecollection->edges.child[k]);
    }

    //Printing out the site table in a way readable by python
    fprintf(sitefilepointer, "position ancestral_state\n");
    for (int k = 0; k < treesequencetablecollection->sites.num_rows; k++) {
        fprintf(sitefilepointer, "%f 0.0\n", treesequencetablecollection->sites.position[k]);
    }

    //Printing out the mutation table in a way readable by python.
    fprintf(mutationfilepointer, "site node derived_state\n");
    for (int k = 0; k < treesequencetablecollection->mutations.num_rows; k++) {
        fprintf(mutationfilepointer, "%d %d %.12s\n", treesequencetablecollection->mutations.site[k], treesequencetablecollection->mutations.node[k], (treesequencetablecollection->mutations.derived_state + k*12));
    }

    // Closes all of the files for storing the tree sequence table data
    fclose(nodefilepointer);
    fclose(edgefilepointer);
    fclose(sitefilepointer);
    fclose(mutationfilepointer);

    // Frees the names of the files
    free(nodefilename);
    free(edgefilename);
    free(sitefilename);
    free(mutationfilename);
}
